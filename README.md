...
Interactive. It will not let you finish until you supply a falsifiable question (`the_nutrient`) that reads like an actual test, not a restatement of the failure. Four independent checks, all enforced, not just suggested:

- **Density**: word count of `blade + nutrient + grain` must be at least the word count of `break`. A long, careful description of what broke raises your own bar — that's deliberate, but it means the fix for a rejection is to make the nutrient and grain denser, not to shorten the break.
- **No padding**: any field that's mostly one repeated character, or mostly one repeated word, is rejected outright. Length you generated by stalling doesn't count as density.
- **No copying**: if the nutrient is a near-verbatim copy of the break (just append a `?` and it still fails), it's rejected. Restating the failure as a question is not the same as compressing it into a testable one.
- **Falsifiability**: the nutrient must contain `?` and at least one real auxiliary verb or comparator (`is`/`does`/`can`/`will`/`before`/`more than`, etc.) matched as a whole word — not as a substring of some other word.
